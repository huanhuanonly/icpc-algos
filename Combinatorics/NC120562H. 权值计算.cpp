 ...
 * 
 * Submissions ~> https://ac.nowcoder.com/acm/contest/view-submission?submissionId=82049643 By huanhuanonly
 * 
 * @b Combinatorics and @b Segment-tree
 * 
 * ----February 05, 2026 [15h:36m:58s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        LazySegmentTree<uint64> seg(n);
        seg.build(std::views::iota(1, n + 1), [](uint64 i) -> uint64 { return i * (i + 1) / 2; });

        std::map<int, int> last;

        for (int i = 0; i < n; ++i)
        {
            if (auto fit = last.find(v[i]); fit != last.end())
            {
                seg.range_update(i, n - 1, -(fit->second + 1));
            }
        ...
        seg.for_each([&ans, i = n](uint64 value) mutable -> bool
        {
            ans += value * i--;
            return true;
        });
