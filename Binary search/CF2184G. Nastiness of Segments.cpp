 ...
 * 
 * => G. Nastiness of Segments ( @c 1900 )
 * 
 * -> https://codeforces.com/contest/2184/problem/G
 * 
 * Submissions ~> https://codeforces.com/contest/2184/submission/359354561 By huanhuanonly
 * 
 * @b Binary-search and @b Segment-tree
 * 
 * ----January 23, 2026 [21h:46m:12s]----
*/
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        SegmentTree<int, decltype(std::ranges::min)> seg(n);
        seg.fill(0, [](int i) -> int { std::cin >> i; return i; });
        ...
        while (q--)
        {
            ...
            else
            {
                ...
                auto range = std::views::iota(0, r - l + 1);
                auto it = std::ranges::lower_bound(range, true, {}, [&](int d) -> bool
                {
                    return seg.reduce(l, l + d) <= d;
                });

                std::cout << (it != range.end() and seg.reduce(l, l + *it) == *it) << '\n';
            }
        }
    }
