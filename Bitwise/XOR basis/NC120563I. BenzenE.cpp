 ...
 * 
 * Submissions ~> https://ac.nowcoder.com/acm/contest/view-submission?submissionId=82173920 By huanhuanonly
 * 
 * @b XOR-basis
 * 
 * ----February 07, 2026 [17h:11m:51s]----
*/
...
public:
    ...
    constexpr bool
    insert_with(value_type __value, int index) noexcept
    {
        std::set<int> ids{index};
    ...
    [[nodiscard]] constexpr std::set<int>
    find_ids(value_type __value) const noexcept
    {
        std::set<int> ids;
...
private:
    ...
    std::array<std::set<int>, bit_count()> _M_ids;
    size_type _M_size;
    bool _M_zero;
};
...
int main(void)
{
    ...
    while (_--)
    {
        ...
        if (sum)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                basis.insert_with(a[i] ^ b[i], i);
            }

            for (const auto& i : basis.find_ids(sum))
            {
                a[i] = b[i];
            }
        }
        
        if (std::reduce(a.begin(), a.end(), uint32{}, std::bit_xor{}) == 0)
        {
            for (int i = 0; i < n; ++i)
            {
                std::cout << a[i] << " \n"[i + 1 == n];
